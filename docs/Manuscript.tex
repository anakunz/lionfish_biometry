...
\author[1]{Juan Carlos Villase̱or-Derbez\(^1\), Sean Fitzgerald}
...
parameters were calculated from filed observations in the central
Mexican Caribbean.
...
was unspecified. Studies inconsistently defined \(a\) as either the
...
conversions. Here, all parameters are reported as TL(mm) to TW(gr)
conversions. When required, values from other studies were transformed
for consistency. Locations where allometric studies have been performed
...
weight estimates by estimating TW for the 109 length observations of
lionfish from the central Mexican Caribbean with each of the 18
...
of variance (ANOVA). All analyses were performed in R version 3.4.4
...
differences observed in age-at-length may be driven by climate, and not
by ecological processes. Differences in weight-at-length could also
reflect differences in energy input (\emph{i.e.} in some regions,
lionfish eat more) or differential usage of this energy (\emph{e.g.}
regional differences in predator abundances lead to different usage of
energy), or a combination of both. Future research should focus on
identifying which occurs here.
...
\citet{wilson_2012} shows that small-scale variations in other
Scorpaeniformes (\emph{Sebastes rastrelliger}) translate to differential
...
the manager desires to erradicate species. To accurately estimate
lionfish removal efforts and allocate resources to manage the invasion,
we must aknowledge and understand the regional biological differences.
