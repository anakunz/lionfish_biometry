...
We appreciate the professional way in which our manuscript has been handled. Here we address each issue, suggestion, and comment raised by yourself and both reviewers. We include the original comment in plain text, and present our responses in italics.
...
- *We chose to keep the word "major" in L26 (now L30) to maintain consistency with the authors' original wording, but we have included quotations to denote this.*

- *In L142 (Now L150) We now say that "ex situ parameters can result in erroneous weight estimates"*

- *Ass suggested, we have changed "major" from L175 to "key" (now L206).*
...
*The text now correctly states the research was performed in the Bahamas (L37).*
...
*We have improved the wording in the original sentence and no longer use the word "know"/"known" (L59).*
...
*The Bahamas are now reported as part of the Western Atlantic Ocean. Text in L82 (now L80-81) and Table 2 (now Table 1) reflect these changes.*
...
*We have modified this so it reads "with TL ranging from 34 mm to 310 mm" in what is now L119.*
...
*We have included the URL in the main text (L128), but the decision to leave it there depends on the editorial formatting. However, PeerJ has an "Additional Information and Declarations" section with a subsection on "Data Availability", where we expect the URL to be included.*
...
<!-- OK I actually think this is still a very confusing paragraph. It still feels unclear what means +/- SDs we're reporting. Perhaps something like this will clarify: "There were significant differences in our predicted weights for the central Mexican Caribbean when using each of the different pairs of parameters (F17;1943 = 24:96; p < 0:001). For example, the actual observed weights of the 109 lionfish from the Central Mexican Caribbean had a mean +/- SD of 52.56 +/- 76.58 g. However, if we used allometric parameter values from Banco Chinchorro in the Caribbean to predict weights from our observed length observations, we estimated a mean +/ SD of 40.37  58.74 g (Sabido-Itza´ et al., 2016). If we similarly used parameter values from NORTH CAROLINA (?) to estimate lionfish weights in the Central Mexican Caribbean, we found a mean +/- SD of 73.76  96.11 g (Barbour et al., 2011). These were the two most extreme sets of parameters, but results consistently varied across sites. Weights predicted from these extreme parameters corresponded to predicted-to-observed weight ratios of 0.80  0.19 and 1.76  0.50 (mean  SD), respectively, and the under- and overestimations FOR LIONFISH THAT WERE 310 MM IN TL (the maximum TL in this study) (??) resulted in ratios of 0.36 and 3.51 of the actual observed weight. Ex situ parameter use can therefore result in highly erroneous weight estimates for lionfish. -->
...
<!-- You gotta tell me how to pronouce Xcalak -->
### Discussion:
...
*Yes, we refer to the spatial heterogeneity. We have modified the text so it reads "...lionfish exhibit highly variable, spatially heterogeneous allometric relationships across their invaded range, and..."*
...
*We also expected estimates from spineless weight to underestimate of total weight, however this wasn't the case, and as you noted data from Fogg et al. (2013) produced some of the largest estimates. After a careful evaluation of their paper, we noted that their Table 1, in fact, reports TW to TL conversions by gender and for pooled genders. The corresponding modifications have been made in the text. Their paper can be found in this link: https://lionfish.co/wp-content/uploads/2013/05/Distribution-of-Lionfish-in-the-Gulf-of-Mexico.pdf*

<!-- I don't think you satisfactorily answered this one - you kind of skirted the reviewer's question of how you adjusted for spineless weight (the answer is we didn't, right?).  Don't think you can get away with just saying "The corresponding modifications have been made in the text." -->
...
<!-- Be more specific than "touches on this" - quote exactly what you wrote to alleviate the reviewer's concerns -->

<!-- Also I think I said "you" a lot instead of "the reviewer" - my bad.  Change it to whatever you want, but make it consistent -->
...
<!-- Don't you think we should include the Central Mexican Caribbean in Fig 3 and Fig S1? -->
...
*As stated above, these were also raised by Rev1. We believe that our modifications to the discussion in lines... now explicitly mention these issues. And while we cannot test for the influence of all these factors on the length-weight relationship, we can still show that using ex situ parameters that follow different sampling collections and without a standardized collection date may lead to over- and underestimation of total weight.*
<!-- modification in which lines? -->
<!-- Alright we gotta do a better job on this one - s/he says this is THE area for major improvement. I think the text is mostly there in the manuscript now, but we need to specifically answer each of the questions the reviewer asks, preferably while quoting the text as well -->
...
<!-- I don't understand the logic in the first sentence - why does collecting them elimiate gear selectivity concerns? -->
...
<!-- Maybe this reviewer would disagree with my idea to add the ANCOVA equation... What do you think? -->
...
<!-- Ok well given this, I guess also ignore my comment that I thought we should include this study in Fig 3 and S1 -->
...
<!-- I can't knit for some reason, so I can't tell if this was done but I assume you took care of it.  Also isn't it b, not c? -->
...
*Figure 3 shows a total of 18 lines. This is because from the 12 reviewed papers, a total of 17 relationships were identified BECAUSE OF SEX-SPECIFIC REPORTING AND SOME STUDIES REPORTING MULTIPLE LOCATIONS?? (if so, say that). The additional relationship is the one estimated in this study.*

<!-- Oh wait - you do include this study in Fig 3 - so ignore BOTH of my previous comments haha- but the line for this study needs to be in the figure legend! -->
