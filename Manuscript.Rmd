...
Studies quantifying biomass from visual surveys

Objectives

  - Provide a new site-specific pair of growth parameters
  - Highlight the relevance of using site-specific parameters
  - Provide a review of existing growth parameters
...
```{r, results = "asis"}
total <- data.frame(Location = "Total", Latitude = "", Longitude = "", N = 109, stringsAsFactors = F)
...
fish %>% 
  ...
  summarize(Latitude = formatC(mean(Latitude), digits = 3, format = "f"),
            ...
            N = n()) %>% 
  rbind(total) %>% 
  stargazer(type = "html", header = F, summary = F, add.lines = list(c("Total", "", "", 109)), rownames = F)
```
...
$$log_{10}(TW) = blog_{10}TL + log{10}(a)$$
...
predicted_all <- data.frame(Length = seq(min(fish$Length), max(fish$Length), by = 1)) %>% 
  mutate(This = 10^a*Length^b, 
         APA = (10^review$a[1]) * ((Length/10)^review$b[1]),
         APF = (10^review$a[2]) * (Length^review$b[2]),
         APM = (10^review$a[3]) * (Length^review$b[3]),
         Sandel = review$a[4] * ((Length/10)^review$b[4]),
         Chin = (10^review$a[5]) * (Length^review$b[5]),
         Barbour = review$a[6] * Length^review$b[6],
         deLeon = review$a[7] * Length^review$b[7],
         FoggA = (10^review$a[8]) * (Length^review$b[8]),
         FoggM = (10^review$a[9]) * (Length^review$b[9]),
         FoggF = (10^review$a[10]) * (Length^review$b[10]),
         Edwards = review$a[11] * Length^review$b[11],
         Sabido = review$a[12] * ((Length/10)^review$b[12]),
         FishBase = review$a[13] * ((Length/10)^review$b[13])) %>%
